...
The normal owner-approval workflow remains the default. The historical Owner Delegated Continuous Execution exception for roadmap Phases 5–10 is retained for traceability; those phases are complete and it is not the active queue. After the Project #3 governance migration merges, the owner-authorized `PROJECT_DRIVEN_DELEGATED_EXECUTION` mode permits internal consistency review to replace routine PLAN/SPEC/PHASE/TODO pauses across the canonical Project #3 queue. It does not authorize silent behavior/API/security drift, production or provider-control-plane mutation, credentials, destructive work, unresolved semantic/architecture decisions, blocking-finding dispositions, reviewer-quorum overrides, or any stop condition in `docs/CHANGE_CONTEXT_AND_REVIEW.md` §10.1.2. A hard-gated item is marked Blocked and independent Ready work may continue; the queue is not empty merely because one item merges.

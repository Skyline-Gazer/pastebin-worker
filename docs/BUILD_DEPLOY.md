...
Production Worker name is `pastebin-feishu-prod`. The initial public origin is the discovered `*.workers.dev` URL for that Worker; do not invent a custom Feishu hostname. Deploy uses `downstream/addons/feishu/wrangler.toml` as the binding contract and a production overlay for D1/queue resource IDs. Set `FEISHU_INGRESS_DLQ_CONFIGURED=true` only after the consumer dead-letter queue is verified. Pastebin origin for the Add-on is `https://pb.223.im`. Do not modify `pastebin-prod` when deploying the Add-on.
